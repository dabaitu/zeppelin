...
public class NotebookServer extends WebSocketServlet
    ...
    try {
      ...
      switch (messagereceived.op) {
        ...
        case RELOAD_NOTES_FROM_REPO:
          broadcastReloadedNoteList(conn, subject, userAndRoles);
          break;
        case GET_HOME_NOTE:
  ...
  public void broadcastReloadedNoteList(NotebookSocket conn, AuthenticationInfo subject,
    ...
    if (conn != null) {
      try {
        ...
        if (!ElfOwl.isSuperUser(userAndRoles)) {
      ...
      if (config.containsKey("releaseresource")) {
        try {
          ...
          if ((releaseResource == true) &&
              !ElfOwl.isSuperUser(userAndRoles)
          ) {
  ...
  private void runParagraph(NotebookSocket conn, HashSet<String> userAndRoles, Notebook notebook,
    ...
    // elfowl
    String replName = Paragraph.getRequiredReplName(text);
    ...
    org.apache.zeppelin.user.UserCredentials userCredentials = note.getCredentials()
        .getUserCredentials(conn.getUser());

    if (replName != null && (replName.contains("vertica") || replName.contains("mysql"))) {
      if (userCredentials == null ||
          userCredentials.getUsernamePassword(replName) == null) {
        LOG.info("User Credentials for user {} not found for data source {}",
            conn.getUser(), replName);
        conn.send(serializeMessage(new Message(OP.AUTH_INFO).put("info",
            "User credentials for data source " + replName + " not found. \n" +
