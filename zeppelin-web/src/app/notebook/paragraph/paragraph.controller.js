...
angular.module('zeppelinWebApp')
  ...
  $scope.getExecutionTime = function() {
    ...
    var desc = 'Took ' +
      ...
      '. Last execution from ' + moment(pdata.dateStarted).format('MMMM DD YYYY, h:mm:ss A') +
      ' to ' + moment(pdata.dateFinished).format('MMMM DD YYYY, h:mm:ss A') + '.';
    if ($scope.isResultOutdated()){
      desc += ' (outdated)';
    }
    return desc;
  ...
  var setTable = function(type, data, refresh) {
    ...
    var renderTable = function() {
      ...
      if (typeof $scope.curSelectCol === 'undefined') {
        $scope.columnFormat = [];
        for (var i = 0; i < columnNames.length; i++) {
          $scope.columnFormat.push('text');
        }
      }
      var handsontable = new Handsontable(container, {
        ...
        cells: function(row, col, prop) {
          ...
          cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
            if (!isNaN(value)) {
              if (typeof $scope.isChangingColType !== 'undefined' && $scope.columnFormat[col] === 'text') {
