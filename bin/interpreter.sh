...
elif [[ "${SCALDING_MODE}" == "local" ]]; then
    ${ZEPPELIN_RUNNER} ${JAVA_INTP_OPTS} ${ZEPPELIN_INTP_MEM} -cp ${SCALDING_EXTRA_LIBS}:${ZEPPELIN_CLASSPATH_OVERRIDES}:${CLASSPATH} ${ZEPPELIN_SERVER} ${PORT} &
elif [[ "${SCALDING_MODE}" == "hdfs" ]]; then
    ${ZEPPELIN_RUNNER} ${JAVA_INTP_OPTS} ${ZEPPELIN_INTP_MEM} -cp `hadoop classpath`:${SCALDING_EXTRA_LIBS}:${ZEPPELIN_CLASSPATH_OVERRIDES}:${CLASSPATH} ${ZEPPELIN_SERVER} ${PORT} &
else
